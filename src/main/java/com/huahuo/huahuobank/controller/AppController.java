...
public class AppController {
    ...
    public ResponseResult upload(MultipartFile file, @RequestParam Integer type, @RequestParam Integer taskId, @RequestParam String num) throws UnsupportedEncodingException {
        String filename = URLDecoder.decode(minioService.upload(file, taskId, num, type), "utf-8");
    ...
    public ResponseResult checkThree(@RequestBody FinalCheckDto dto) {
        ...
        task.setGetCarTime(DateUtil.formatDate(DateUtil.parse(DateUtil.now())));
    ...
    public ResponseResult manage(@RequestBody ManageDto dto) {
        ...
        task.setThirdSubTime(DateUtil.formatDate(DateUtil.parse(DateUtil.now())));
    ...
    public ResponseResult pass(@RequestBody PassDto dto) {
        ...
        switch (dto.getType()) {
            case 1:
                ...
                task.setFirstPassTime(DateUtil.formatDate(DateUtil.parse(DateUtil.now())));
            ...
            case 2:
                ...
                task.setSecondPassTime(DateUtil.formatDate(DateUtil.parse(DateUtil.now())));
                task.setSecondPassUser(user.getNickname());
                task.setGpsSituation("已贴G");
                break;
            case 3:
                task.setIsRemarkThree(1);
                task.setThirdPassTime(DateUtil.formatDate(DateUtil.parse(DateUtil.now())));
            ...
            case 4:   //分期
                ...
                manageDetail.setIsClear(1);
                task.setPrincipal(task.getPrincipal() - manageDetail.getPrice());
                task.setIsClear(3); //分期中
                task.setIsRemarkFour(0);
                task.setIsRemarkThree(0);
                queryWrapper.eq(ManageDetail::getIsClear, 2);
                manageDetailService.updateById(manageDetail);
                if (task.getPrincipal() <= 0) {
                    task.setIsClear(1);
                    task.setIsRemarkFour(1);  //已处置
                    task.setForthPassTime(DateUtil.formatDate(DateUtil.parse(DateUtil.now())));
                    task.setForthPassUser(user.getNickname());
                    task.setCarSituation("已处置");
